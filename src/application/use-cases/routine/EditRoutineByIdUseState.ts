...
export class EditRoutineByIdUseState {
    ...
    async execute(routine_id: number, updateData: Partial<RoutinesSchema>) : Promise<RoutinesSchema | null> {
        const routine = await this.routineRepository.editRoutineById(routine_id, updateData);
